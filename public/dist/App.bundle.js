...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    var places = res.data;
    if (!places.length) {
      // TODO: create an error message for user
      console.log('no places found');
      return;
    }

    // create a bounds so that we don't have to adjust the Zoom level
    // and the map will be perfectly centered around any available markers
    ...
    var markers = places.map(function (place) {
      ...
      // pt. 2 setting bounds for map
      bounds.extend(position);
      var marker = new google.maps.Marker({ map: map, position: position });
      // Add information from our API to the marker
      marker.place = place;
      return marker;
    });

    // when someone clicks on a marker, show details for that places
    // .addListener is how we can add events with Google Maps
    ...
    // Zoom the map to fit markers perfectly
...
// import typeAhead for searching
